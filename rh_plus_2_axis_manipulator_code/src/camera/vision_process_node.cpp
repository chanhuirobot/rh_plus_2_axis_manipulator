...
void VisionNode::image_process_callback(sensor_msgs::msg::Image::SharedPtr data)
{
  ...
  // 만약 빨간점이 발견되면 최신화. 거꾸로 말하면 발견 안되면 최신화 안함.
  if(nlabels > 1){
  ...
  cv::imshow("output", img_warp); // 딱 필요한 화면(자르기 완료 + 사각형 점 등 시각처리)
  cv::waitKey(1);
}
...
MoveItController::MoveItController() : Node("moveit_node"){
  // 15초마다 moveit2_controller 발동시키기
  this->moveit_timer_ = this->create_wall_timer(std::chrono::seconds(15), std::bind(&MoveItController::moveit2_controller, this));
...
void MoveItController::moveit2_controller(){
  ...
  for(int countdown_num = 10; countdown_num > 0; countdown_num--){
    RCLCPP_INFO(get_logger(), "Countdown for next Moving: %ds / current coord ratio : (x = %.3f, y = %.3f)", countdown_num, VisionNode::coord_x_ratio_, VisionNode::coord_y_ratio_);
  ...
  double catch_coord_x_ratio = VisionNode::coord_x_ratio_;
  double catch_coord_y_ratio = VisionNode::coord_y_ratio_;
  target_pose.position.x = catch_coord_x_ratio * BOARD_WIDTH;
  target_pose.position.y = catch_coord_y_ratio * BOARD_HEIGHT;
  ...
  if(success){
    ...
    RCLCPP_INFO(get_logger(), "Success! Coord ratio : (x = %.3f, y = %.3f)", catch_coord_x_ratio, catch_coord_y_ratio);
  }
  else RCLCPP_ERROR(get_logger(), "Planning failed! Maybe Out of Range...! :( ratio : (x = %.3f, y = %.3f)", catch_coord_x_ratio, catch_coord_y_ratio);
}
